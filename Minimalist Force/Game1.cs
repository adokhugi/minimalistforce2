...
namespace WindowsGame2
{
    ...
    public class Game1 : Microsoft.Xna.Framework.Game
    {
        ...
        enum GameModes
        {
            ...
            NextEnemyCharacterAfterSleepMode,
        ...
        protected override void Initialize()
        {
            ...
            Character.TexturesNeutralTower = this.Content.Load<Texture2D>("mini_neutral_tower");
            Character.TexturesPartyTower = this.Content.Load<Texture2D>("mini_party_tower");
            Character.TexturesEnemiesTower = this.Content.Load<Texture2D>("mini_enemies_tower");
        ...
        protected override void Draw(GameTime gameTime)
        {
            if (!lockDraw)
            {
                ...
                switch (GameMode)
                {
                    ...
                    case GameModes.NextEnemyCharacterAfterSleepMode:
                        Draw_NextCharacterAfterSleepMode(gameTime);
                        break;
        ...
        private void UpdateVariables()
        {
            switch (GameMode)
            {
                ...
                case GameModes.NextEnemyCharacterAfterSleepMode:
                    UpdateVariables_NextEnemyCharacterAfterSleepMode();
                    break;
                    ...
                    else
                    {
                              ...
                              && selectedMagicSpell.Type == Spell.Types.Recruit)
                        {
                            ...
                            switch (newPartyMember.Name)
                            {
                                ...
                                case "TOWER":
                                    newPartyMember.Texture = Character.TexturesPartyTower;
                                    break;
                            }
        ...
        private void UpdateVariables_NextEnemyCharacterAfterSleepMode()
        {
                    ...
                    else
                    {
                        ...
                        else if (enemyBattleMove == BattleMoves.MagicRecruit)
                        {
                            ...
                            switch (newPartyMember.Name)
                            {
                                ...
                                case "TOWER":
                                    newPartyMember.Texture = Character.TexturesEnemiesTower;
                                    break;
                            }
...
//            EnterGeneralMenuMode();
        ...
        private void EnterPlayerMoveMode()
        {
            ...
            else if (party.Members[party.MemberOnTurn].Name == "TOWER")
            {
                var res = ProducePartyCharacter(party);
                textMessage[0] = res ? "TOWER produces warrior." : "TOWER is idle.";
                textMessageLength = 1;
                returnGameMode = GameModes.NextCharacterAfterSleepMode;
                waitForKeyPress = true;
                moveOut = true;
        ...
        private bool ProducePartyCharacter(Party this_party)
        {
            //1234567890
            if (random.Next(100) % 4 != 0) return false;

            var pos = this_party.Members[this_party.MemberOnTurn].Position;
            var posVec = map.CalcPosition(this_party.Members[this_party.MemberOnTurn].Position);
            var newPos = -1.0f;
            if (!map.AtLeftBorder(posVec) 
                && !map.AnyCharacterLocatedAt(pos - 1, party)
                && !map.AnyCharacterLocatedAt(pos - 1, enemies)
                && !map.AnyCharacterLocatedAt(pos - 1, neutral_characters))
            {
                newPos = pos - 1;
            }
            else if (!map.AtRightBorder(posVec)
                && !map.AnyCharacterLocatedAt(pos + 1, party)
                && !map.AnyCharacterLocatedAt(pos + 1, enemies)
                && !map.AnyCharacterLocatedAt(pos + 1, neutral_characters))
            {
            ...
            else if (!map.AtTopBorder(posVec)
                && !map.AnyCharacterLocatedAt(pos - map.Size.X, party)
                && !map.AnyCharacterLocatedAt(pos - map.Size.X, enemies)
                && !map.AnyCharacterLocatedAt(pos - map.Size.X, neutral_characters))
            {
                newPos = pos - map.Size.X;
            }
            else if (!map.AtBottomBorder(posVec)
                && !map.AnyCharacterLocatedAt(pos + map.Size.X, party)
                && !map.AnyCharacterLocatedAt(pos + map.Size.X, enemies)
                && !map.AnyCharacterLocatedAt(pos + map.Size.X, neutral_characters))
            {
            ...
            if (newPos >= 0)
            {
                var newCharacter = new Character("WARRIOR");
                newCharacter.Position = newPos;
                if (this_party == party)
                {
                    party.Members.Add(newCharacter);
                    var i = party.Members.Count - 1;
                    mapPositionPlayerMoveMode[i] = map.CenterMapPosition((int)party.Members[i].Position);
                }
                else if (this_party == enemies)
                {
                    newCharacter.Texture = Character.TexturesBanana;
                    enemies.Members.Add(newCharacter);
                    var i = enemies.Members.Count - 1;
                    mapPositionEnemyMoveMode[i] = map.CenterMapPosition((int)enemies.Members[i].Position);
                }
        ...
        private void EnterEnemyMoveMode()
        {
            ...
            else
            {
                ...
                if (enemies.Members[enemies.MemberOnTurn].Name == "TOWER")
                {
                    var res = ProducePartyCharacter(enemies);
                    textMessage[0] = res ? "TOWER produces warrior." : "TOWER is idle.";
                    textMessageLength = 1;
                    returnGameMode = GameModes.NextEnemyCharacterAfterSleepMode;
                    waitForKeyPress = true;
                    moveOut = true;
                    EnterDisplayTextMessageMode();
                }
                else if (enemies.Members[enemies.MemberOnTurn].Confused > 0)
                {
        ...
        private void EnterInitializeBattle1Mode()
        {
            ...
            for (int i = 0; i < 10; i++)
            {
                switch (rnd.Next(4))
                {
                    ...
                    case 3:
                        neutral_characters.Members.Add(new Character("N.TOWER"));
                        break;
                }
